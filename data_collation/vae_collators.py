...
from args_configs.model_args import ModelArguments
from feature_extraction import normalize_mel_spectrogram, rereference_mel_db
from transformers.models.wav2vec2.modeling_wav2vec2 import _compute_mask_indices


def normalize_vae_mel_batch(batch, model_args, model_name, spec_max, frame_len, device, select_component=0):
    """
    Normalize mel features that were extracted per utterance at the preprocessing step, for the
    VAE-based models. Preprocessing keeps every decomposition component, so each is re-referenced
    against the batch dB peak and normalized on its own before one of them is selected. Modifies
    batch in place, and must run before the mask length is derived from the input length.

    Args:
        batch (dict): The padded batch, holding input_values
        model_args (:class:`~args_configs.model_args.ModelArguments`): Holds the VAE mel settings
            (vae_input_type, n_mels_vae, mel_norm_vae)
        model_name (str): The VAE type - the sequence models are flattened back after normalizing
        spec_max (list): Per-utterance reference peaks stored at preprocessing, one per component
        frame_len (list): Per-utterance frame length, used by the sequence models
        device (str or torch.device): Device of the returned tensors
        select_component (int or None): Component to keep. None keeps them all, which the ICA/PCA
            evaluations need.
    """
    if not model_args.vae_input_type.startswith('mel'):
        return

    "Preprocessing stores a reference peak whenever it extracts. Without one nothing was extracted"
    "for this model type - VAE_1D consumes the raw sequence - so there is nothing to normalize"
    if not spec_max or not all(s is not None for s in spec_max):
        return

    stored_spec_max = torch.tensor(np.asarray(spec_max, dtype=np.float32), dtype=batch["input_values"].dtype)

    values = batch["input_values"]
    "VAE_1D_FC_seq is flattened back into a sequence once normalized"
    flat_seq = "seq" in model_name
    feature_length = int(frame_len[0]) if flat_seq else values.shape[-1]

    "Only the frame models carry a component axis - (batch, components, frames, n_mels)"
    if not flat_seq and values.dim() != 4:
        if stored_spec_max is not None:
            values = rereference_mel_db(values, stored_spec_max)
        if model_args.mel_norm_vae is not None:
            values = normalize_mel_spectrogram(
                values,
                normalize=model_args.mel_norm_vae,
    ...
    if flat_seq:
        "The sequence models carry no component axis - preprocessing already selected the signal"
        if stored_spec_max is not None:
            values = rereference_mel_db(values, stored_spec_max)
        if model_args.mel_norm_vae is not None:
            values = normalize_mel_spectrogram(
        ...
        batch["input_values"] = values.reshape(values.shape[0], -1)
        return
    ...
    for o in range(values.shape[1]):
        component = values[:, o, ...]
        if stored_spec_max is not None:
            component = rereference_mel_db(component, stored_spec_max[:, o])
        if model_args.mel_norm_vae is not None:
    ...
    batch["input_values"] = values if select_component is None else values[:, select_component, ...]
...
@dataclass
class DataCollatorForVAE1DPreTraining_NoFeatureExtraction:
    """
    Data collator to be used when the features have already been extracted at the preprocessing step
    by prepare_extract_features_vae_pretraining_dataset. Pads the inputs, aligns them with labels and
    variables, prepares masked indices, and normalizes the mel features here, where the whole batch
    is visible. VAEs operate on a single signal, so there is no component axis.
    ...
    Args:
        ...
        model_args (:class:`~args_configs.model_args.ModelArguments`): The model arguments dictionary;
            the VAE mel settings (vae_input_type, n_mels_vae, mel_norm_vae) are needed here.
        model_name (:obj:`str`, `optional`, defaults to :obj:`"VAE_1D"`): The name of the model being used.
        padding (:obj:`bool`, :obj:`str` or :class:`~transformers.tokenization_utils_base.PaddingStrategy`, `optional`, defaults to :obj:`True`):
        ...
        mask_time_length (:obj:`int`, `optional`, defaults to :obj:`10`):
            Length of each vector mask span to mask along the time axis.
    """
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        ...
        "Pop decomposed features, keep only X. Preprocessing consumes the sequence itself for the"
        "models whose features are framed there, so select it here only when it is still present"
        if (self.model_name == "VAE_1D" or "seq" in self.model_name) and features[0].get("input_seq_values") is not None:
            for feature in features:
                new_input = feature.pop("input_seq_values")[0]
                feature["input_values"] = new_input
        else:
            for feature in features:
                feature.pop("input_seq_values", None)

        spec_max = [feature.pop("mel_spec_max", None) for feature in features]
        frame_len = [feature.pop("mel_frame_len", None) for feature in features]
        ...
        "Normalize mel features - the features were extracted per utterance at preprocessing."
        "Runs before the mask length is derived, since it changes the input length."
        normalize_vae_mel_batch(batch, self.model_args, self.model_name, spec_max, frame_len, device)

        "VAE_1D_FC consumed a single component of the decomposition; for mel the normalization"
        "above already selected it, and fft kept every component as it did in the training loop"
        if self.model_name == "VAE_1D_FC" and self.model_args.vae_input_type.startswith("waveform"):
...
@dataclass
class DataCollatorForVAE1D_SSL_FineTuning_NoFeatureExtraction:
    """
    Data collator to be used when the features have already been extracted at the preprocessing
    step by prepare_extract_features_vae_pretraining_dataset. Pads the inputs, aligns labels and
    other variables, prepares masked indices for self-supervised fine-tuning (fine-tuning using a
    VAE loss), and normalizes the mel features here, where the whole batch is visible.
...
@dataclass
class DataCollatorForVAE1DLatentPostAnalysis_NoFeatureExtraction:
    """
    Data collator to be used when the features have already been extracted at the preprocessing
    step. Pads the inputs, aligns them with labels and variables, and normalizes the mel features
    here, where the whole batch is visible. Every decomposition component is kept, because the
    ICA/PCA evaluations select components themselves. Used for post-training latent evaluations
    of a VAE-based model.
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        ...
        "Normalize mel features extracted at preprocessing - all components are kept so the"
        "ICA/PCA evaluations can select the ones they need"
        normalize_vae_mel_batch(batch, self.model_args, self.model_name, spec_max, frame_len,
                                device, select_component=None)
...
@dataclass
class DataCollatorForVAE1DLatentTraversals_NoFeatureExtraction:
    """
    Data collator to be used when the features have already been extracted at the preprocessing
    step. Pads the inputs, aligns labels and other variables, and normalizes the mel features here,
    where the whole batch is visible. Every decomposition component is kept, because the traversal
    script selects components itself.
    for post-training latent traversal analysis for a VAE-based model.
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        ...
        "Normalize mel features extracted at preprocessing - all components are kept so the"
        "traversal script can select the ones it needs"
        normalize_vae_mel_batch(batch, self.model_args, self.model_name, spec_max, frame_len,
                                device, select_component=None)
