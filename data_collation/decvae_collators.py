...
from feature_extraction import extract_mel_spectrogram, extract_fft_psd, normalize_mel_spectrogram, rereference_mel_db
import numpy as np


def normalize_mel_batch(batch, input_type, data_training_args, device):
    """
    ...
    matching what the collators that extract the features themselves do. Modifies batch in place.

    Args:
        batch (dict): The padded batch, holding input_values and optionally input_seq_values
        input_type (str): The type of input features - normalization only applies to 'mel'
        data_training_args (:class:`~args_configs.data_training_args.DataTrainingArguments`): Holds
            the mel settings (mel_norm, n_mels)
        device (str or torch.device): Device of the returned tensors
    """
    if input_type != 'mel':
        return

    "Preprocessing referenced each utterance against its own peak - restore the batch peak"
    spec_max = batch.pop("mel_spec_max", None)
    seq_spec_max = batch.pop("mel_seq_spec_max", None)

    def _renormalize(key, stored_spec_max):
        feature_length = batch[key].shape[-1]
        components = []
        for o in range(batch[key].shape[1]):
            component = batch[key][:,o,...]
            if stored_spec_max is not None:
                component = rereference_mel_db(component, stored_spec_max[:,o])
            if data_training_args.mel_norm is not None:
                component = normalize_mel_spectrogram(
                    component,
                    normalize=data_training_args.mel_norm,
                    feature_length=feature_length,
                    device=device,
                    n_mels=data_training_args.n_mels,
                )
            components.append(component)
        batch[key] = torch.stack(components, dim=1)

    _renormalize("input_values", spec_max)
    if batch.get("input_seq_values") is not None:
        _renormalize("input_seq_values", seq_spec_max)
...
@dataclass
class DataCollatorForDecVAE_SSL_FineTuning_NoFeatureExtraction:
    """
    Data collator to be used when the features have already been extracted at the preprocessing
    step. Pads the inputs received, aligns them with labels and variables, prepares masked indices
    for self-supervised fine-tuning of a Variational Decomposition Autoencoder, and normalizes the
    mel features here, where the whole batch is visible. Supports slightly different functionality to
    account for datasets that are used only for fine-tuning e.g. VOC-ALS.
...
@dataclass
class DataCollatorForDecVAELatentPostAnalysis_NoFeatureExtraction:
    """
    Data collator to be used when the features have already been extracted at the preprocessing
    step. Pads the inputs received, aligns labels and other variables, and normalizes the mel
    features here, where the whole batch is visible. Prepares masked indices for the post-training
    evaluation of a Variational Decomposition Autoencoder. Supports slightly different model and data
    training arguments through ModelArgumentsPost and DataTrainingArgumentsPost.
...
@dataclass
class DataCollatorForDecVAELatentTraversals_NoFeatureExtraction:
    """
    Data collator to be used when the features have already been extracted at the preprocessing
    step. Pads the inputs received, aligns labels and other variables, and normalizes the mel
    features here, where the whole batch is visible. Prepares masked indices for prompting a Variational
    Decomposition Autoencoder for latent disentanglement traversals extraction. Supports slightly
    different model and data training arguments through ModelArgumentsPost and DataTrainingArgumentsPost.
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        ...
        "Features were extracted at the preprocessing step - only normalize them here."
        "The dataset-specific reshape the extraction used for the vowels traversals is no longer"
        "needed: it compensated for librosa collapsing the single-frame axis when a whole batch"
        "was passed at once, and preprocessing now extracts one utterance at a time."
        normalize_mel_batch(batch, self.input_type, self.data_training_args, device)
...
@dataclass
class DataCollatorForDecVAELatentVisualization_NoFeatureExtraction:
    """
    Data collator to be used when the features have already been extracted at the preprocessing
    step. Pads the inputs received, aligns labels and other variables, and normalizes the mel
    features here, where the whole batch is visible. Prepares masked indices for prompting a Variational
    Decomposition Autoencoder for latent representations visualization. Supports slightly different
    model and data training arguments through ModelArgumentsPost and DataTrainingArgumentsPost.
