...
"""Fine-tuning (domain adaptation with SSL decomposition loss) a DecVAE model on unlabeled audio data 
with transfer learning using 🤗 Transformers utilities"""
...
def main():
    ...
    # Determine if we should load a pretrained model (for VOC_ALS or IEMOCAP with transfer learning)
    ...
    if config.dual_branched_latent:
        ...
        optimizer_z = AdamW([param_groups[0]],
        ...
        optimizer_s = AdamW([param_groups[1]],
            betas=[data_training_args.adam_beta1, data_training_args.adam_beta2],
            eps=data_training_args.adam_epsilon,
            weight_decay=data_training_args.weight_decay,)
    ...
    for epoch in range(starting_epoch, data_training_args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            #print(batch["input_seq_values"].shape)
            ...
            if (completed_steps+1) % (data_training_args.gradient_accumulation_steps * data_training_args.logging_steps) == 0:
                ...
                if accelerator.state.num_processes > 1:
                    ...
                    if config.dual_branched_latent or config.only_z_branch:
                        ...
                        div_dict_z = {key: accelerator.gather_for_metrics(values.detach()).sum() for key, values in div_dict_z.items()} #/num_losses_div
                    ...
                    if config.dual_branched_latent or config.only_s_branch:
                        ...
                        div_dict_s = {key: accelerator.gather_for_metrics(values.detach()).sum() for key, values in div_dict_s.items()} #/num_losses_div
