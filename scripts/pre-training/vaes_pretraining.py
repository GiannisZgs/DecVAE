...
"""Pre-Training a VAE model on unlabeled audio data. Supports pretraining on SimVowels and TIMIT for now. IEMOCAP is supported 
for fine-tuning only by fine_tune_vaes.py script. VOC-ALS is directly processed in latents_post_analysis_vae1D.py."""
...
def main():
    ...
    eval_dataloader = DataLoader(
        vectorized_datasets["validation"].with_format("numpy"), collate_fn=data_collator, batch_size=data_training_args.per_device_eval_batch_size
    )
    ...
    for epoch in range(starting_epoch, data_training_args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if data_training_args.dataset_name in ["timit"]:
