...
from latent_analysis_utils import visualize, set_vis_flags, manifold, plot_manifold_latents
import warnings
import time
        ...
        def sim_vowels_latent_vis(config,data_training_args,decomp_args,data_subset,vis_that_subset,
            ...
            if vis_that_subset and vis_args.visualize_latent_frame and (config.dual_branched_latent or config.only_z_branch):
                ...
                if "vowel" in vis_args.variables_to_plot_latent:
                    ...
                    if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_frame:
                        ...
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_originals_z, OCs=mu_components_z, y_vec=vowel_labels, target="vowel",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='X_OCs', group='vowels')
                    if "OCs_joint" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs joint embedding (concatenation) - Vowels & Frequency"
                        data_training_args.frequency_vis = False
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_OCs_joint_frame_reduced, OCs=None, y_vec=vowel_labels, target="vowel",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='OCs_joint_emb', group='vowels')
                    if "OCs_proj" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs projection - Vowels & Frequency"
                        if config.project_OCs:
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_projections_z, OCs=None, y_vec=vowel_labels, target="vowel",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                manifold_dict=manifold_dict, embedding='OCs_projection', group='vowels')
                    if "all" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        "TSNE - All / total embedding (X + OCs) - Vowels & Frequency"

                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_all_frame_reduced, OCs=None, y_vec=vowel_labels, target="vowel",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='all_joint_emb', group='vowels')
                    ...
                    manifold_dict = manifold('tsne', vis_args, metric='cosine', n_components=3)
                    if vis_args.plot_3d:
                        if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_frame:
                    ...
                    if vis_args.use_umap:
                        ...
                        data_training_args.tsne_plot_2d_3d = '3d'
                        data_training_args.vis_sphere= vis_args.vis_isotropic_gaussian_sphere
                        manifold_dict = manifold('umap', vis_args, metric='cosine', n_components=3)
                        if vis_args.plot_3d:
                            if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_frame:
                                "UMAP - X / OCs - Vowels & Frequency"    
                                plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                ...
                if "speaker_id" in vis_args.variables_to_plot_latent:
                    ...
                    if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_frame:
                        ...
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_originals_z, OCs=mu_components_z, y_vec=speaker_labels_frame,
                            target="speaker_frame",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='X_OCs', group='speakers')

                    if "OCs_joint" in vis_args.aggregation_strategies_to_plot_frame:
                        "TSNE - OCs joint embedding (concatenation) - Vowels & Frequency"
                        data_training_args.frequency_vis = False
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_OCs_joint_frame_reduced, OCs=None, y_vec=speaker_labels_frame,
                            target="speaker_frame",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='OCs_joint_emb', group='speakers')
                    ...
                    if "OCs_proj" in vis_args.aggregation_strategies_to_plot_frame:
                        ...
                        if config.project_OCs:
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_projections_z, OCs=None, y_vec=speaker_labels_frame, target="speaker_frame",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                manifold_dict=manifold_dict, embedding='OCs_projection', group='speakers')
                    ...
                    if "all" in vis_args.aggregation_strategies_to_plot_frame:
                        ...
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_all_frame_reduced, OCs=None, y_vec=speaker_labels_frame, target="speaker_frame",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='all_joint_emb', group='speakers')

                    "--------------------------------------------------------------------------------------------"
                    "3D TSNE Speaker Visualizations"
                    "--------------------------------------------------------------------------------------------"
                    set_vis_flags(data_training_args, vis_method='tsne', vis_sphere=vis_args.vis_isotropic_gaussian_sphere, plot_2d_3d='3d')
                    manifold_dict = manifold('tsne', vis_args, metric='cosine', n_components=3)
                    if vis_args.plot_3d:
                        if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_frame:
                            "TSNE - X / OCs - Vowels & Frequency"
                        ...
                        if "OCs_proj" in vis_args.aggregation_strategies_to_plot_frame:
                            ...
                            if config.project_OCs:
                                plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                    X=mu_projections_z, OCs=None, y_vec=speaker_labels_frame,
                                    target="speaker_frame",
                                    data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                    manifold_dict=manifold_dict, embedding='OCs_projection', group='speakers')
                        if "all" in vis_args.aggregation_strategies_to_plot_frame:
                            ...
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_all_frame_reduced, OCs=None, y_vec=speaker_labels_frame,
                                target="speaker_frame",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                manifold_dict=manifold_dict, embedding='all_joint_emb', group='speakers')
                    ...
                    if vis_args.use_umap:
                        ...
                        "--------------------------------------------------------------------------------------------"
                        set_vis_flags(data_training_args, vis_method='umap')
                        ...
                        "--------------------------------------------------------------------------------------------"
                        set_vis_flags(data_training_args, vis_method='umap', vis_sphere=vis_args.vis_isotropic_gaussian_sphere, plot_2d_3d='3d')
                        manifold_dict = manifold('umap', vis_args, metric='cosine', n_components=3)
                        if vis_args.plot_3d:
                            if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_frame:
                                "UMAP - X / OCs - Vowels & Frequency" 
            ...
            if vis_that_subset and vis_args.visualize_latent_sequence and (config.dual_branched_latent or config.only_s_branch):
                ...
                if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_seq:
                    ...
                    plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                        X=mu_originals_s, OCs=mu_components_s, y_vec=speaker_labels_seq, target="speaker_seq",
                        data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                        manifold_dict=manifold_dict, embedding='X_OCs', group='speakers_seq')

                if "OCs_joint" in vis_args.aggregation_strategies_to_plot_seq:
                    data_training_args.frequency_vis = False
                    "TSNE - OCs joint embedding (concatenation) - Speakers Sequence"
                    plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                        X=mu_OCs_joint_seq_reduced, OCs=None, y_vec=speaker_labels_seq, target="speaker_seq",
                        data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                        manifold_dict=manifold_dict, embedding='OCs_joint_emb', group='speakers_seq')

                if "OCs_proj" in vis_args.aggregation_strategies_to_plot_seq:
                    data_training_args.frequency_vis = False
                    "TSNE - OCs projection - Speakers Sequence"
                    plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                        X=mu_projections_s, OCs=None, y_vec=speaker_labels_seq, target="speaker_seq",
                        data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                        manifold_dict=manifold_dict, embedding='OCs_projection', group='speakers_seq')

                if "all" in vis_args.aggregation_strategies_to_plot_seq:
                    data_training_args.frequency_vis = False
                    "TSNE - All / total embedding (X + OCs) - Speakers Sequence"
                    plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                        X=mu_all_seq_reduced, OCs=None, y_vec=speaker_labels_seq, target="speaker_seq",
                        data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                        manifold_dict=manifold_dict, embedding='all_joint_emb', group='speakers_seq')

                "--------------------------------------------------------------------------------------------------------------"
                "3D TSNE - Speakers Sequence"
                "--------------------------------------------------------------------------------------------------------------"

                set_vis_flags(data_training_args, vis_method='tsne', frequency_vis=True, vis_sphere=vis_args.vis_isotropic_gaussian_sphere, plot_2d_3d='3d')

                manifold_dict = manifold('tsne', vis_args, metric='canberra', perplexity=15, n_components=3)
                if vis_args.plot_3d:
                    if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_seq:
                ...
                if vis_args.use_umap:
                    ...
                    set_vis_flags(data_training_args, vis_method='umap', frequency_vis=True)
                    ...
                    set_vis_flags(data_training_args, vis_method='umap', frequency_vis=True, vis_sphere=vis_args.vis_isotropic_gaussian_sphere, plot_2d_3d='3d')
                    manifold_dict = manifold('umap', vis_args, metric='canberra', n_neighbors=15, min_dist=0.9)
                    if vis_args.plot_3d:
                        if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_seq:    
                            "UMAP - X - OCs - Speakers Sequence"
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_originals_s, OCs=mu_components_s, y_vec=speaker_labels_seq,
                                target="speaker_seq",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                                manifold_dict=manifold_dict, embedding='X_OCs', group='speakers_seq')
                        ...
                        if "OCs_joint" in vis_args.aggregation_strategies_to_plot_seq:
                            ...
                            "UMAP - OCs joint embedding (concatenation) - Speakers Sequence"
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_OCs_joint_seq_reduced, OCs=None, y_vec=speaker_labels_seq,
                                target="speaker_seq",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                                manifold_dict=manifold_dict, embedding='OCs_joint_emb', group='speakers_seq')
        ...
        def timit_latent_vis(config,data_training_args,decomp_args,data_subset,vis_that_subset,
            ...
            if vis_that_subset and vis_args.visualize_latent_frame and (config.dual_branched_latent or config.only_z_branch):
                ...
                if "phoneme" in vis_args.variables_to_plot_latent:
                    ...
                    if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_frame:
                        ...
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_originals_z_sel_phonemes, OCs=mu_components_z_sel_phonemes, y_vec=sel_phonemes,
                            target="phoneme39",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='X_OCs', group='phonemes')
                    if "OCs_joint" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs joint embedding (concatenation) - Phonemes "
                        data_training_args.frequency_vis = False
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_OCs_joint_frame_reduced_sel_phonemes, OCs=None, y_vec=sel_phonemes,
                            target="phoneme39",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='OCs_joint_emb', group='phonemes')
                    if "OCs_proj" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs projection - Phonemes"
                        if config.project_OCs:
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_projections_z_sel_phonemes, OCs=None, y_vec=sel_phonemes, target="phoneme39",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                manifold_dict=manifold_dict, embedding='OCs_projection', group='phonemes')
                    if "all" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        "TSNE - All / total embedding (X + OCs) - Phonemes"

                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_all_frame_reduced_sel_phonemes, OCs=None, y_vec=sel_phonemes, target="phoneme39",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='all_joint_emb', group='phonemes')
                    ...
                    if vis_args.plot_3d:
                        ...
                        if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_frame:
                            ...
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_originals_z_sel_phonemes, OCs=mu_components_z_sel_phonemes,
                                y_vec=sel_phonemes, target="phoneme39",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                manifold_dict=manifold_dict, embedding='X_OCs', group='phonemes')
                        if "OCs_joint" in vis_args.aggregation_strategies_to_plot_frame:
                            "TSNE - OCs joint embedding (concatenation) - Phonemes"
                        ...
                        if "OCs_proj" in vis_args.aggregation_strategies_to_plot_frame:
                            ...
                            if config.project_OCs:
                                plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                    X=mu_projections_z_sel_phonemes, OCs=None, y_vec=sel_phonemes,
                                    target="phoneme39",
                                    data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                    manifold_dict=manifold_dict, embedding='OCs_projection', group='phonemes')
                        if "all" in vis_args.aggregation_strategies_to_plot_frame:
                            ...
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_all_frame_reduced_sel_phonemes, OCs=None, y_vec=sel_phonemes,
                                target="phoneme39",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                manifold_dict=manifold_dict, embedding='all_joint_emb', group='phonemes')
                ...
                if "consonant" in vis_args.variables_to_plot_latent:
                    ...
                    if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_frame:
                        ...
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_originals_z_sel_consonants, OCs=mu_components_z_sel_consonants,
                            y_vec=sel_consonants, target="consonant",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='X_OCs', group='consonants')
                    if "OCs_joint" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs joint embedding (concatenation) - Consonants & Frequency"
                        data_training_args.frequency_vis = False
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_OCs_joint_frame_reduced_sel_consonants, OCs=None, y_vec=sel_consonants,
                            target="consonant",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='OCs_joint_emb', group='consonants')
                    if "OCs_proj" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs projection - Consonants & Frequency"
                        if config.project_OCs:
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_projections_z_sel_consonants, OCs=None, y_vec=sel_consonants,
                                target="consonant",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                manifold_dict=manifold_dict, embedding='OCs_projection', group='consonants')
                    if "all" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        "TSNE - All / total embedding (X + OCs) - Consonants & Frequency"

                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_all_frame_reduced_sel_consonants, OCs=None, y_vec=sel_consonants,
                            target="consonant",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='all_joint_emb', group='consonants')
                            ...
                            if "OCs_proj" in vis_args.aggregation_strategies_to_plot_frame:
                                ...
                                if config.project_OCs:
                                    plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                        ...
                                        target="consonant",
                                        data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                        manifold_dict=manifold_dict, embedding='OCs_projection',
                                        group='consonants')
                ...
                if "vowel" in vis_args.variables_to_plot_latent:
                    ...
                    if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_frame:
                        "TSNE - X / OCs - Vowels & Frequency"  
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_originals_z_sel_vowels, OCs=mu_components_z_sel_vowels, y_vec=sel_vowels,
                            target="vowel",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='X_OCs', group='vowels')
                    if "OCs_joint" in vis_args.aggregation_strategies_to_plot_frame:
                        ...
                        data_training_args.frequency_vis = False
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_OCs_joint_frame_reduced_sel_vowels, OCs=None, y_vec=sel_vowels, target="vowel",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='OCs_joint_emb', group='vowels')
                    if "OCs_proj" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        ...
                        if config.project_OCs:
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_projections_z_sel_vowels, OCs=None, y_vec=sel_vowels, target="vowel",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                manifold_dict=manifold_dict, embedding='OCs_projection', group='vowels')
                    if "all" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        ...
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_all_frame_reduced_sel_vowels, OCs=None, y_vec=sel_vowels, target="vowel",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='all_joint_emb', group='vowels')
                    ...
                    if vis_args.plot_3d:
                        ...
                        if "OCs_joint" in vis_args.aggregation_strategies_to_plot_frame:
                            ...
                            data_training_args.frequency_vis = False
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_OCs_joint_frame_reduced_sel_vowels, OCs=None, y_vec=sel_vowels,
                                target="vowel",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                manifold_dict=manifold_dict, embedding='OCs_joint_emb', group='vowels')
                        if "OCs_proj" in vis_args.aggregation_strategies_to_plot_frame:
                ...
                if "speaker_id" in vis_args.variables_to_plot_latent:
                    ...
                    if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_frame:
                        "TSNE - X / OCs - Vowels & Frequency" 
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_originals_z_sel_speakers, OCs=mu_components_z_sel_speakers, y_vec=sel_10_speakers,
                            target="speaker_frame",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='X_OCs', group='speakers')
                    ...
                    if "OCs_joint" in vis_args.aggregation_strategies_to_plot_frame:
                        ...
                        data_training_args.frequency_vis = False
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_OCs_joint_frame_reduced_sel_speakers, OCs=None, y_vec=sel_10_speakers,
                            target="speaker_frame",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='OCs_joint_emb', group='speakers')
                    ...
                    if "OCs_proj" in vis_args.aggregation_strategies_to_plot_frame:
                        ...
                        if config.project_OCs:
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_projections_z_sel_speakers, OCs=None, y_vec=sel_10_speakers,
                                target="speaker_frame",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                manifold_dict=manifold_dict, embedding='OCs_projection', group='speakers')
                    ...
                    if "all" in vis_args.aggregation_strategies_to_plot_frame:
                        ...
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_all_frame_reduced_sel_speakers, OCs=None, y_vec=sel_10_speakers,
                            target="speaker_frame",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='all_joint_emb', group='speakers')
                    ...
                    if vis_args.plot_3d:
                        ...
                        if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_frame:
                            "TSNE - X / OCs - Vowels & Frequency"                            
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_originals_z, OCs=mu_components_z_sel_speakers, y_vec=sel_10_speakers,
                                target="speaker_frame",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                manifold_dict=manifold_dict, embedding='X_OCs', group='speakers')
                    ...
                    if vis_args.use_umap:
                        ...
                        if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_frame:
                            "UMAP - X / OCs - Vowels & Frequency"  
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_originals_z_sel_speakers, OCs=mu_components_z_sel_speakers,
                                y_vec=sel_10_speakers, target="speaker_frame",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                manifold_dict=manifold_dict, embedding='X_OCs', group='speakers')
            ...
            if vis_that_subset and vis_args.visualize_latent_sequence and (config.dual_branched_latent or config.only_s_branch):
                ...
                if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_seq:
                    "TSNE - X - OCs - Speakers Sequence" 
                    plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                        X=mu_originals_s_sel_speakers, OCs=mu_components_s_sel_speakers, y_vec=sel_10_speakers_seq,
                        target="speaker_seq",
                        data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                        manifold_dict=manifold_dict, embedding='X_OCs', group='speakers_seq')
                ...
                if "OCs_joint" in vis_args.aggregation_strategies_to_plot_seq:
                    ...
                    "TSNE - OCs joint embedding (concatenation) - Speakers Sequence"
                    plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                        X=mu_OCs_joint_seq_reduced_sel_speakers, OCs=None, y_vec=sel_10_speakers_seq,
                        target="speaker_seq",
                        data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                        manifold_dict=manifold_dict, embedding='OCs_joint_emb', group='speakers_seq')
                if "OCs_proj" in vis_args.aggregation_strategies_to_plot_seq:
                    ...
                    "TSNE - OCs projection - Speakers Sequence"
                    plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                        X=mu_projections_s_sel_speakers, OCs=None, y_vec=sel_10_speakers_seq, target="speaker_seq",
                        data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                        manifold_dict=manifold_dict, embedding='OCs_projection', group='speakers_seq')
                ...
                if "all" in vis_args.aggregation_strategies_to_plot_seq:
                    ...
                    "TSNE - All / total embedding (X + OCs) - Speakers Sequence"
                    plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                        X=mu_all_seq_reduced_sel_speakers, OCs=None, y_vec=sel_10_speakers_seq,
                        target="speaker_seq",
                        data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                        manifold_dict=manifold_dict, embedding='all_joint_emb', group='speakers_seq')
                ...
                if vis_args.plot_3d:
                    ...
                    if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_seq:
                        "TSNE - X - OCs - Speakers Sequence" 
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_originals_s_sel_speakers, OCs=mu_components_s_sel_speakers,
                            y_vec=sel_10_speakers_seq, target="speaker_seq",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                            manifold_dict=manifold_dict, embedding='X_OCs', group='speakers_seq')
                    ...
                    if "OCs_proj" in vis_args.aggregation_strategies_to_plot_seq:
                        ...
                        "TSNE - OCs projection - Speakers Sequence"
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_projections_s_sel_speakers, OCs=None, y_vec=sel_10_speakers_seq,
                            target="speaker_seq",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                            manifold_dict=manifold_dict, embedding='OCs_projection', group='speakers_seq')
                    if "all" in vis_args.aggregation_strategies_to_plot_seq:
            ...
            if vis_that_subset and vis_args.visualize_latent_frame and (config.dual_branched_latent or config.only_z_branch):
                ...
                if "phoneme" in vis_args.variables_to_plot_latent:
                    ...
                    if "OCs_proj" in vis_args.aggregation_strategies_to_plot_frame:
                        ...
                        if config.project_OCs:
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_projections_z_sel_phonemes, OCs=None, y_vec=sel_phonemes, target="phoneme",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                manifold_dict=manifold_dict, embedding='OCs_projection', group='phonemes')
                    if "all" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        ...
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_all_frame_reduced_sel_phonemes, OCs=None, y_vec=sel_phonemes, target="phoneme",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='all_joint_emb', group='phonemes')
                    ...
                    if vis_args.plot_3d:
                        ...
                        if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_frame:
                            ...
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_originals_z_sel_phonemes, OCs=mu_components_z_sel_phonemes,
                                y_vec=sel_phonemes, target="phoneme",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                manifold_dict=manifold_dict, embedding='X_OCs', group='phonemes')
                        if "OCs_joint" in vis_args.aggregation_strategies_to_plot_frame:
                            "TSNE - OCs joint embedding (concatenation) - Phonemes"
                ...
                if "non_verbal_phoneme" in vis_args.variables_to_plot_latent:
                    ...
                    if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_frame:
                        ...
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_originals_z_sel_non_verbal_phonemes, OCs=mu_components_z_sel_non_verbal_phonemes,
                            y_vec=sel_non_verbal_phonemes, target="non_verbal_phoneme",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='X_OCs', group='non_verbal_phonemes')
                    if "OCs_joint" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs joint embedding (concatenation) - Non-verbal Phonemes"
                        data_training_args.frequency_vis = False
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_OCs_joint_frame_reduced_sel_non_verbal_phonemes, OCs=None,
                            y_vec=sel_non_verbal_phonemes, target="non_verbal_phoneme",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='OCs_joint_emb', group='non_verbal_phonemes')
                    if "OCs_proj" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs projection - Non-verbal Phonemes"
                        if config.project_OCs:
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_projections_z_sel_non_verbal_phonemes, OCs=None,
                                y_vec=sel_non_verbal_phonemes, target="non_verbal_phoneme",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                manifold_dict=manifold_dict, embedding='OCs_projection',
                                group='non_verbal_phonemes')
                    if "all" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        "TSNE - All / total embedding (X + OCs) - Non-verbal Phonemes"

                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_all_frame_reduced_sel_non_verbal_phonemes, OCs=None,
                            y_vec=sel_non_verbal_phonemes, target="non_verbal_phoneme",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='all_joint_emb', group='non_verbal_phonemes')
                    ...
                    if vis_args.plot_3d:
                        ...
                        if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_frame:
                            ...
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                ...
                                OCs=mu_components_z_sel_non_verbal_phonemes, y_vec=sel_non_verbal_phonemes,
                        ...
                        if "OCs_joint" in vis_args.aggregation_strategies_to_plot_frame:
                            ...
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                ...
                                y_vec=sel_non_verbal_phonemes, target="non_verbal_phoneme",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                manifold_dict=manifold_dict, embedding='OCs_joint_emb',
                                group='non_verbal_phonemes')
                        if "OCs_proj" in vis_args.aggregation_strategies_to_plot_frame:
                            data_training_args.frequency_vis = False
                        ...
                        if "all" in vis_args.aggregation_strategies_to_plot_frame:
                            ...
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                ...
                                y_vec=sel_non_verbal_phonemes, target="non_verbal_phoneme",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                manifold_dict=manifold_dict, embedding='all_joint_emb',
                                group='non_verbal_phonemes')
                ...
                if "emotion" in vis_args.variables_to_plot_latent:
                    ...
                    if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_frame:
                        "TSNE - X / OCs - Emotions"  
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_originals_z, OCs=mu_components_z, y_vec=emotion_labels_frame,
                            target="emotion_frame",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='X_OCs', group='categorical_emotions')

                    if "OCs_joint" in vis_args.aggregation_strategies_to_plot_frame:
                        "TSNE - OCs joint embedding (concatenation) - Emotions"
                        data_training_args.frequency_vis = False
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_OCs_joint_frame_reduced, OCs=None, y_vec=emotion_labels_frame,
                            target="emotion_frame",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='OCs_joint_emb', group='categorical_emotions')
                    ...
                    if "OCs_proj" in vis_args.aggregation_strategies_to_plot_frame:
                        ...
                        if config.project_OCs:
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_projections_z, OCs=None, y_vec=emotion_labels_frame, target="emotion_frame",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                manifold_dict=manifold_dict, embedding='OCs_projection',
                                group='categorical_emotions')
                    ...
                    if "all" in vis_args.aggregation_strategies_to_plot_frame:
                        ...
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_all_frame_reduced, OCs=None, y_vec=emotion_labels_frame, target="emotion_frame",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='all_joint_emb', group='categorical_emotions')
                    ...
                    if vis_args.plot_3d:
                        ...
                        if "OCs_proj" in vis_args.aggregation_strategies_to_plot_frame:
                            ...
                            if config.project_OCs:
                                plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                    X=mu_projections_z, OCs=None, y_vec=emotion_labels_frame,
                                    target="emotion_frame",
                                    data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                    manifold_dict=manifold_dict, embedding='OCs_projection',
                                    group='categorical_emotions')
                        if "all" in vis_args.aggregation_strategies_to_plot_frame:
                            ...
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_all_frame_reduced, OCs=None, y_vec=emotion_labels_frame,
                                target="emotion_frame",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                manifold_dict=manifold_dict, embedding='all_joint_emb',
                                group='categorical_emotions')
            ...
            if vis_that_subset and vis_args.visualize_latent_sequence and (config.dual_branched_latent or config.only_s_branch):
                ...
                if "emotion" in vis_args.variables_to_plot_latent_seq:
                    ...
                    if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_seq:
                        ...
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_originals_s, OCs=mu_components_s, y_vec=emotion_labels_seq, target="emotion_seq",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                            manifold_dict=manifold_dict, embedding='X_OCs', group='emotion_seq')

                    if "OCs_joint" in vis_args.aggregation_strategies_to_plot_seq:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs joint embedding (concatenation) - Emotion Sequence"
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_OCs_joint_seq_reduced, OCs=None, y_vec=emotion_labels_seq, target="emotion_seq",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                            manifold_dict=manifold_dict, embedding='OCs_joint_emb', group='emotion_seq')
                    if "OCs_proj" in vis_args.aggregation_strategies_to_plot_seq:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs projection - Emotion Sequence"
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_projections_s, OCs=None, y_vec=emotion_labels_seq, target="emotion_seq",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                            manifold_dict=manifold_dict, embedding='OCs_projection', group='emotion_seq')

                    if "all" in vis_args.aggregation_strategies_to_plot_seq:
                        data_training_args.frequency_vis = False
                        "TSNE - All / total embedding (X + OCs) - Emotion Sequence"
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_all_seq_reduced, OCs=None, y_vec=emotion_labels_seq, target="emotion_seq",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                            manifold_dict=manifold_dict, embedding='all_joint_emb', group='emotion_seq')
                    ...
                    if vis_args.plot_3d:
                        ...
                        if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_seq:
                            "TSNE - X - OCs - Emotion Sequence" 
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_originals_s, OCs=mu_components_s, y_vec=emotion_labels_seq,
                                target="emotion_seq",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                                manifold_dict=manifold_dict, embedding='X_OCs', group='emotion_seq')
                        ...
                        if "OCs_joint" in vis_args.aggregation_strategies_to_plot_seq:
                            ...
                            "TSNE - OCs joint embedding (concatenation) - Emotion Sequence"
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_OCs_joint_seq_reduced, OCs=None, y_vec=emotion_labels_seq,
                                target="emotion_seq",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                                manifold_dict=manifold_dict, embedding='OCs_joint_emb', group='emotion_seq')
            ...
            if vis_that_subset and vis_args.visualize_latent_frame and (config.dual_branched_latent or config.only_z_branch):
                ...
                if "phoneme" in vis_args.variables_to_plot_latent:
                    ...
                    if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_frame:
                        "TSNE - X / OCs - Phonemes & Frequency" 
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_originals_z, OCs=mu_components_z, y_vec=phoneme_labels, target="phoneme39",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='X_OCs', group='phonemes')
                    if "OCs_joint" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        ...
                        data_training_args.frequency_vis = False
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_OCs_joint_frame_reduced, OCs=None, y_vec=phoneme_labels, target="phoneme",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='OCs_joint_emb', group='phonemes')
                    if "OCs_proj" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        ...
                        if config.project_OCs:
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_projections_z, OCs=None, y_vec=phoneme_labels, target="phoneme",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                manifold_dict=manifold_dict, embedding='OCs_projection', group='phonemes')
                    if "all" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        ...
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_all_frame_reduced, OCs=None, y_vec=phoneme_labels, target="phoneme",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='all_joint_emb', group='phonemes')
                    ...
                    if vis_args.plot_3d:
                        ...
                        if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_frame:
                            "TSNE - X / OCs - Phonemes & Frequency - 3D sphere"    
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_originals_z, OCs=mu_components_z, y_vec=phoneme_labels, target="phoneme",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                manifold_dict=manifold_dict, embedding='X_OCs', group='phonemes')
                        if "OCs_joint" in vis_args.aggregation_strategies_to_plot_frame:
                            "TSNE - OCs joint embedding (concatenation) - Phonemes"
                ...
                if "alsfrs_total" in vis_args.variables_to_plot_latent:
                    ...
                    if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_frame:
                        ...
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_originals_z, OCs=mu_components_z, y_vec=alsfrs_total, target="alsfrs_total",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='X_OCs', group='alsfrs_total')
                    if "OCs_joint" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs joint embedding (concatenation) - ALSFRS-Total"
                        data_training_args.frequency_vis = False
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_OCs_joint_frame_reduced, OCs=None, y_vec=alsfrs_total, target="alsfrs_total",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='OCs_joint_emb', group='alsfrs_total')
                    if "OCs_proj" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs projection - ALSFRS-Total"
                        if config.project_OCs:
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_projections_z, OCs=None, y_vec=alsfrs_total, target="alsfrs_total",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                manifold_dict=manifold_dict, embedding='OCs_projection', group='alsfrs_total')
                    if "all" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        "TSNE - All / total embedding (X + OCs) - ALSFRS-Total"

                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_all_frame_reduced, OCs=None, y_vec=alsfrs_total, target="alsfrs_total",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='all_joint_emb', group='alsfrs_total')
                    ...
                    if vis_args.use_umap:
                        ...
                        if vis_args.plot_3d:
                            ...
                            if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_frame:
                                "UMAP - X / OCs - ALSFRS-Total & Frequency"
                                plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                    X=mu_originals_z, OCs=mu_components_z, y_vec=alsfrs_total,
                                    target="alsfrs_total",
                                    data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                    manifold_dict=manifold_dict, embedding='X_OCs', group='alsfrs_total')
                            if "OCs_joint" in vis_args.aggregation_strategies_to_plot_frame:
                                ...
                                "UMAP - OCs joint embedding (concatenation) - ALSFRS-Total"
                                plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                    X=mu_OCs_joint_frame_reduced, OCs=None, y_vec=alsfrs_total,
                                    target="alsfrs_total",
                                    data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                    manifold_dict=manifold_dict, embedding='OCs_joint_emb', group='alsfrs_total')
                            if "OCs_proj" in vis_args.aggregation_strategies_to_plot_frame:
                ...
                if "alsfrs_speech" in vis_args.variables_to_plot_latent:
                    ...
                    if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_frame:
                        "TSNE - X / OCs - ALSFRS-Total & Frequency"
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_originals_z, OCs=mu_components_z, y_vec=alsfrs_speech, target="alsfrs_speech",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='X_OCs', group='alsfrs_speech')
                    if "OCs_joint" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs joint embedding (concatenation) - ALSFRS-Total"
                        data_training_args.frequency_vis = False
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_OCs_joint_frame_reduced, OCs=None, y_vec=alsfrs_speech, target="alsfrs_speech",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='OCs_joint_emb', group='alsfrs_speech')
                    if "OCs_proj" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs projection - ALSFRS-Total"
                        if config.project_OCs:
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_projections_z, OCs=None, y_vec=alsfrs_speech, target="alsfrs_speech",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                manifold_dict=manifold_dict, embedding='OCs_projection', group='alsfrs_speech')
                    if "all" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        "TSNE - All / total embedding (X + OCs) - ALSFRS-Total"

                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_all_frame_reduced, OCs=None, y_vec=alsfrs_speech, target="alsfrs_speech",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='all_joint_emb', group='alsfrs_speech')
                    ...
                    if vis_args.plot_3d:
                        ...
                        if "OCs_joint" in vis_args.aggregation_strategies_to_plot_frame:
                            ...
                            data_training_args.frequency_vis = False
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_OCs_joint_frame_reduced, OCs=None, y_vec=alsfrs_speech,
                    ...
                    if vis_args.use_umap:
                        ...
                        if vis_args.plot_3d:
                            ...
                            if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_frame:
                                "UMAP - X / OCs - ALSFRS-Total & Frequency"  
                                plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                    X=mu_originals_z, OCs=mu_components_z, y_vec=alsfrs_speech,
                                    target="alsfrs_speech",
                                    data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                    manifold_dict=manifold_dict, embedding='X_OCs', group='alsfrs_speech')
                            if "OCs_joint" in vis_args.aggregation_strategies_to_plot_frame:
                ...
                if "disease_duration" in vis_args.variables_to_plot_latent:
                    ...
                    if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_frame:
                        ...
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_originals_z, OCs=mu_components_z, y_vec=disease_duration,
                            target="disease_duration",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='X_OCs', group='disease_duration')
                    if "OCs_joint" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs joint embedding (concatenation) - Disease Duration"
                        data_training_args.frequency_vis = False
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_OCs_joint_frame_reduced, OCs=None, y_vec=disease_duration,
                            target="disease_duration",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='OCs_joint_emb', group='disease_duration')
                    if "OCs_proj" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs projection - Disease Duration"
                        if config.project_OCs:
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_projections_z, OCs=None, y_vec=disease_duration, target="disease_duration",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                manifold_dict=manifold_dict, embedding='OCs_projection', group='disease_duration')
                    if "all" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        "TSNE - All / total embedding (X + OCs) - Disease Duration"

                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_all_frame_reduced, OCs=None, y_vec=disease_duration, target="disease_duration",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='all_joint_emb', group='disease_duration')
                    ...
                    if vis_args.plot_3d:
                        ...
                        if "OCs_proj" in vis_args.aggregation_strategies_to_plot_frame:
                            ...
                            if config.project_OCs:
                                plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                    X=mu_projections_z, OCs=None, y_vec=disease_duration,
                                    target="disease_duration",
                                    data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                    manifold_dict=manifold_dict, embedding='OCs_projection',
                        ...
                        if "all" in vis_args.aggregation_strategies_to_plot_frame:
                            ...
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_all_frame_reduced, OCs=None, y_vec=disease_duration,
                                target="disease_duration",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                manifold_dict=manifold_dict, embedding='all_joint_emb', group='disease_duration')
                ...
                if "king_stage" in vis_args.variables_to_plot_latent:
                    ...
                    if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_frame:
                        ...
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_originals_z, OCs=mu_components_z, y_vec=king_stage, target="king_stage",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='X_OCs', group='king_stage')
                    if "OCs_joint" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs joint embedding (concatenation) - King's stage"
                        data_training_args.frequency_vis = False
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_OCs_joint_frame_reduced, OCs=None, y_vec=king_stage, target="king_stage",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='OCs_joint_emb', group='king_stage')
                    if "OCs_proj" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs projection - King's stage"
                        if config.project_OCs:
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_projections_z, OCs=None, y_vec=king_stage, target="king_stage",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                manifold_dict=manifold_dict, embedding='OCs_projection', group='king_stage')
                    if "all" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        "TSNE - All / total embedding (X + OCs) - King's stage"

                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_all_frame_reduced, OCs=None, y_vec=king_stage, target="king_stage",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='all_joint_emb', group='king_stage')
                ...
                if "group" in vis_args.variables_to_plot_latent:
                    ...
                    if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_frame:
                        ...
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_originals_z, OCs=mu_components_z, y_vec=group, target="group",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='X_OCs', group='group')
                    if "OCs_joint" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs joint embedding (concatenation) - Disease category / group"
                        data_training_args.frequency_vis = False
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_OCs_joint_frame_reduced, OCs=None, y_vec=group, target="group",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='OCs_joint_emb', group='group')
                    if "OCs_proj" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs projection - Disease category / group"
                        if config.project_OCs:
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_projections_z, OCs=None, y_vec=group, target="group",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                manifold_dict=manifold_dict, embedding='OCs_projection', group='group')
                    if "all" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        "TSNE - All / total embedding (X + OCs) - Disease category / group"

                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_all_frame_reduced, OCs=None, y_vec=group, target="group",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='all_joint_emb', group='group')
                ...
                if "cantagallo" in vis_args.variables_to_plot_latent:
                    ...
                    if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_frame:
                        ...
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_originals_z, OCs=mu_components_z, y_vec=cantagallo, target="cantagallo",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='X_OCs', group='cantagallo')
                    if "OCs_joint" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs joint embedding (concatenation) - Cantagallo"
                        data_training_args.frequency_vis = False
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_OCs_joint_frame_reduced, OCs=None, y_vec=cantagallo, target="cantagallo",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='OCs_joint_emb', group='cantagallo')
                    if "OCs_proj" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs projection - Cantagallo"
                        if config.project_OCs:
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_projections_z, OCs=None, y_vec=cantagallo, target="cantagallo",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                                manifold_dict=manifold_dict, embedding='OCs_projection', group='cantagallo')
                    if "all" in vis_args.aggregation_strategies_to_plot_frame:
                        data_training_args.frequency_vis = False
                        "TSNE - All / total embedding (X + OCs) - Cantagallo"

                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_all_frame_reduced, OCs=None, y_vec=cantagallo, target="cantagallo",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' + str(vis_args.latent_train_set_frames_to_vis) + '_frames',
                            manifold_dict=manifold_dict, embedding='all_joint_emb', group='cantagallo')
            ...
            if vis_that_subset and vis_args.visualize_latent_sequence and (config.dual_branched_latent or config.only_s_branch):
                ...
                if "phoneme" in vis_args.variables_to_plot_latent_seq:
                    ...
                    if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_seq:
                        ...
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_originals_s, OCs=mu_components_s, y_vec=phoneme_labels_seq, target="phoneme_seq",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                            manifold_dict=manifold_dict, embedding='X_OCs', group='phoneme_seq')

                    if "OCs_joint" in vis_args.aggregation_strategies_to_plot_seq:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs joint embedding (concatenation) - Phoneme Sequence"
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_OCs_joint_seq_reduced, OCs=None, y_vec=phoneme_labels_seq, target="phoneme_seq",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                            manifold_dict=manifold_dict, embedding='OCs_joint_emb', group='phoneme_seq')
                    if "OCs_proj" in vis_args.aggregation_strategies_to_plot_seq:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs projection - Phoneme Sequence"
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_projections_s, OCs=None, y_vec=phoneme_labels_seq, target="phoneme_seq",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                            manifold_dict=manifold_dict, embedding='OCs_projection', group='phoneme_seq')

                    if "all" in vis_args.aggregation_strategies_to_plot_seq:
                        data_training_args.frequency_vis = False
                        "TSNE - All / total embedding (X + OCs) - Phoneme Sequence"
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_all_seq_reduced, OCs=None, y_vec=phoneme_labels_seq, target="phoneme_seq",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                            manifold_dict=manifold_dict, embedding='all_joint_emb', group='phoneme_seq')
                    ...
                    if vis_args.plot_3d:
                        ...
                        if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_seq:
                            "TSNE - X - OCs - Phoneme Sequence"
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_originals_s, OCs=mu_components_s, y_vec=phoneme_labels_seq,
                                target="phoneme_seq",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                                manifold_dict=manifold_dict, embedding='X_OCs', group='phoneme_seq')
                        ...
                        if "OCs_joint" in vis_args.aggregation_strategies_to_plot_seq:
                            ...
                            "TSNE - OCs joint embedding (concatenation) - Phoneme Sequence"
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_OCs_joint_seq_reduced, OCs=None, y_vec=phoneme_labels_seq,
                                target="phoneme_seq",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                                manifold_dict=manifold_dict, embedding='OCs_joint_emb', group='phoneme_seq')
                ...
                if "alsfrs_total" in vis_args.variables_to_plot_latent_seq:
                    ...
                    if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_seq:
                        ...
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_originals_s, OCs=mu_components_s, y_vec=alsfrs_total_seq,
                            target="alsfrs_total_seq",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                            manifold_dict=manifold_dict, embedding='X_OCs', group='alsfrs_total_seq')

                    if "OCs_joint" in vis_args.aggregation_strategies_to_plot_seq:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs joint embedding (concatenation) - ALSFRS-Total Sequence"
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_OCs_joint_seq_reduced, OCs=None, y_vec=alsfrs_total_seq,
                            target="alsfrs_total_seq",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                            manifold_dict=manifold_dict, embedding='OCs_joint_emb', group='alsfrs_total_seq')
                    if "OCs_proj" in vis_args.aggregation_strategies_to_plot_seq:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs projection - Phoneme Sequence"
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_projections_s, OCs=None, y_vec=alsfrs_total_seq, target="alsfrs_total_seq",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                            manifold_dict=manifold_dict, embedding='OCs_projection', group='alsfrs_total_seq')

                    if "all" in vis_args.aggregation_strategies_to_plot_seq:
                        data_training_args.frequency_vis = False
                        "TSNE - All / total embedding (X + OCs) - ALSFRS-Total Sequence"
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_all_seq_reduced, OCs=None, y_vec=alsfrs_total_seq, target="alsfrs_total_seq",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                            manifold_dict=manifold_dict, embedding='all_joint_emb', group='alsfrs_total_seq')
                        ...
                        if vis_args.plot_3d:
                            ...
                            if "OCs_joint" in vis_args.aggregation_strategies_to_plot_seq:
                                ...
                                plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                    ...
                                    data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                                    manifold_dict=manifold_dict, embedding='OCs_joint_emb',
                                    group='alsfrs_total_seq')
                            if "OCs_proj" in vis_args.aggregation_strategies_to_plot_seq:
                                data_training_args.frequency_vis = False
                                "UMAP - OCs projection - ALSFRS-Total Sequence"
                                plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                    X=mu_projections_s, OCs=None, y_vec=alsfrs_total_seq,
                                    target="alsfrs_total_seq",
                                    data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                                    manifold_dict=manifold_dict, embedding='OCs_projection',
                                    group='alsfrs_total_seq')
                            if "all" in vis_args.aggregation_strategies_to_plot_seq:
                                ...
                                "UMAP - All / total embedding (X + OCs) - ALSFRS-Total Sequence"
                                plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                    X=mu_all_seq_reduced, OCs=None, y_vec=alsfrs_total_seq,
                                    target="alsfrs_total_seq",
                                    data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                                    manifold_dict=manifold_dict, embedding='all_joint_emb',
                                    group='alsfrs_total_seq')
                ...
                if "alsfrs_speech" in vis_args.variables_to_plot_latent_seq:
                    ...
                    if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_seq:
                        ...
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_originals_s, OCs=mu_components_s, y_vec=alsfrs_speech_seq,
                            target="alsfrs_speech_seq",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                            manifold_dict=manifold_dict, embedding='X_OCs', group='alsfrs_speech_seq')

                    if "OCs_joint" in vis_args.aggregation_strategies_to_plot_seq:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs joint embedding (concatenation) - ALSFRS-Speech Sequence"
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_OCs_joint_seq_reduced, OCs=None, y_vec=alsfrs_speech_seq,
                            target="alsfrs_speech_seq",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                            manifold_dict=manifold_dict, embedding='OCs_joint_emb', group='alsfrs_speech_seq')
                    if "OCs_proj" in vis_args.aggregation_strategies_to_plot_seq:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs projection - ALSFRS-Speech Sequence"
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_projections_s, OCs=None, y_vec=alsfrs_speech_seq, target="alsfrs_speech_seq",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                            manifold_dict=manifold_dict, embedding='OCs_projection', group='alsfrs_speech_seq')

                    if "all" in vis_args.aggregation_strategies_to_plot_seq:
                        data_training_args.frequency_vis = False
                        "TSNE - All / total embedding (X + OCs) - ALSFRS-Speech Sequence"
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_all_seq_reduced, OCs=None, y_vec=alsfrs_speech_seq, target="alsfrs_speech_seq",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                            manifold_dict=manifold_dict, embedding='all_joint_emb', group='alsfrs_speech_seq')
                    ...
                    if vis_args.plot_3d:
                        ...
                        if "all" in vis_args.aggregation_strategies_to_plot_seq:
                            ...
                            "TSNE - All / total embedding (X + OCs) - ALSFRS-Speech Sequence"
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_all_seq_reduced, OCs=None, y_vec=alsfrs_speech_seq,
                                target="alsfrs_speech_seq",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                                manifold_dict=manifold_dict, embedding='all_joint_emb', group='alsfrs_speech_seq')
                        ...
                        if vis_args.plot_3d:
                            ...
                            if "OCs_joint" in vis_args.aggregation_strategies_to_plot_seq:
                                ...
                                plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                    ...
                                    data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                                    manifold_dict=manifold_dict, embedding='OCs_joint_emb',
                                    group='alsfrs_speech_seq')
                            if "OCs_proj" in vis_args.aggregation_strategies_to_plot_seq:
                                data_training_args.frequency_vis = False
                                "UMAP - OCs projection - ALSFRS-Speech Sequence"
                                plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                    X=mu_projections_s, OCs=None, y_vec=alsfrs_speech_seq,
                                    target="alsfrs_speech_seq",
                                    data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                                    manifold_dict=manifold_dict, embedding='OCs_projection',
                                    group='alsfrs_speech_seq')
                ...
                if "disease_duration" in vis_args.variables_to_plot_latent_seq:
                    ...
                    if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_seq:
                        ...
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_originals_s, OCs=mu_components_s, y_vec=disease_duration_seq,
                            target="disease_duration_seq",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                            manifold_dict=manifold_dict, embedding='X_OCs', group='disease_duration_seq')

                    if "OCs_joint" in vis_args.aggregation_strategies_to_plot_seq:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs joint embedding (concatenation) - Disease Duration Sequence"
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_OCs_joint_seq_reduced, OCs=None, y_vec=disease_duration_seq,
                            target="disease_duration_seq",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                            manifold_dict=manifold_dict, embedding='OCs_joint_emb', group='disease_duration_seq')
                    if "OCs_proj" in vis_args.aggregation_strategies_to_plot_seq:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs projection - Disease Duration Sequence"
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_projections_s, OCs=None, y_vec=disease_duration_seq,
                            target="disease_duration_seq",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                            manifold_dict=manifold_dict, embedding='OCs_projection', group='disease_duration_seq')

                    if "all" in vis_args.aggregation_strategies_to_plot_seq:
                        data_training_args.frequency_vis = False
                        "TSNE - All / total embedding (X + OCs) - Disease Duration Sequence"
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_all_seq_reduced, OCs=None, y_vec=disease_duration_seq,
                            target="disease_duration_seq",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                            manifold_dict=manifold_dict, embedding='all_joint_emb', group='disease_duration_seq')
                ...
                if "king_stage" in vis_args.variables_to_plot_latent_seq:
                    ...
                    if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_seq:
                        ...
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_originals_s, OCs=mu_components_s, y_vec=king_stage_seq, target="king_stage_seq",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                            manifold_dict=manifold_dict, embedding='X_OCs', group='king_stage_seq')

                    if "OCs_joint" in vis_args.aggregation_strategies_to_plot_seq:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs joint embedding (concatenation) - King's stage Sequence"
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_OCs_joint_seq_reduced, OCs=None, y_vec=king_stage_seq, target="king_stage_seq",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                            manifold_dict=manifold_dict, embedding='OCs_joint_emb', group='king_stage_seq')
                    if "OCs_proj" in vis_args.aggregation_strategies_to_plot_seq:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs projection - King's stage Sequence"
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_projections_s, OCs=None, y_vec=king_stage_seq, target="king_stage_seq",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                            manifold_dict=manifold_dict, embedding='OCs_projection', group='king_stage_seq')

                    if "all" in vis_args.aggregation_strategies_to_plot_seq:
                        data_training_args.frequency_vis = False
                        "TSNE - All / total embedding (X + OCs) - King's stage Sequence"
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_all_seq_reduced, OCs=None, y_vec=king_stage_seq, target="king_stage_seq",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                            manifold_dict=manifold_dict, embedding='all_joint_emb', group='king_stage_seq')
                    ...
                    if vis_args.plot_3d:
                        ...
                        if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_seq:
                            ...
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_originals_s, OCs=mu_components_s, y_vec=king_stage_seq,
                        ...
                        if "OCs_joint" in vis_args.aggregation_strategies_to_plot_seq:
                            ...
                            "TSNE - OCs joint embedding (concatenation) - King's stage Sequence"
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_OCs_joint_seq_reduced, OCs=None, y_vec=king_stage_seq,
                                target="king_stage_seq",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                                manifold_dict=manifold_dict, embedding='OCs_joint_emb', group='king_stage_seq')
                ...
                if "group" in vis_args.variables_to_plot_latent_seq:
                    ...
                    if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_seq:
                        ...
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_originals_s, OCs=mu_components_s, y_vec=group_seq, target="group_seq",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                            manifold_dict=manifold_dict, embedding='X_OCs', group='group_seq')

                    if "OCs_joint" in vis_args.aggregation_strategies_to_plot_seq:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs joint embedding (concatenation) - Disease category / group Sequence"
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_OCs_joint_seq_reduced, OCs=None, y_vec=group_seq, target="group_seq",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                            manifold_dict=manifold_dict, embedding='OCs_joint_emb', group='group_seq')
                    if "OCs_proj" in vis_args.aggregation_strategies_to_plot_seq:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs projection - Disease category / group Sequence"
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_projections_s, OCs=None, y_vec=group_seq, target="group_seq",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                            manifold_dict=manifold_dict, embedding='OCs_projection', group='group_seq')

                    if "all" in vis_args.aggregation_strategies_to_plot_seq:
                        data_training_args.frequency_vis = False
                        "TSNE - All / total embedding (X + OCs) - Disease category / group Sequence"
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_all_seq_reduced, OCs=None, y_vec=group_seq, target="group_seq",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                            manifold_dict=manifold_dict, embedding='all_joint_emb', group='group_seq')
                ...
                if "cantagallo" in vis_args.variables_to_plot_latent_seq:
                    ...
                    if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_seq:
                        ...
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_originals_s, OCs=mu_components_s, y_vec=cantagallo_seq, target="cantagallo_seq",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                            manifold_dict=manifold_dict, embedding='X_OCs', group='cantagallo_seq')

                    if "OCs_joint" in vis_args.aggregation_strategies_to_plot_seq:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs joint embedding (concatenation) - Cantagallo Sequence"
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_OCs_joint_seq_reduced, OCs=None, y_vec=cantagallo_seq, target="cantagallo_seq",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                            manifold_dict=manifold_dict, embedding='OCs_joint_emb', group='cantagallo_seq')
                    if "OCs_proj" in vis_args.aggregation_strategies_to_plot_seq:
                        data_training_args.frequency_vis = False
                        "TSNE - OCs projection - Cantagallo Sequence"
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_projections_s, OCs=None, y_vec=cantagallo_seq, target="cantagallo_seq",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                            manifold_dict=manifold_dict, embedding='OCs_projection', group='cantagallo_seq')

                    if "all" in vis_args.aggregation_strategies_to_plot_seq:
                        data_training_args.frequency_vis = False
                        "TSNE - All / total embedding (X + OCs) - Cantagallo Sequence"
                        plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                            X=mu_all_seq_reduced, OCs=None, y_vec=cantagallo_seq, target="cantagallo_seq",
                            data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                            manifold_dict=manifold_dict, embedding='all_joint_emb', group='cantagallo_seq')
                    ...
                    if vis_args.plot_3d:
                        ...
                        if "X_OCs_freq" in vis_args.aggregation_strategies_to_plot_seq:
                            ...
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_originals_s, OCs=mu_components_s, y_vec=cantagallo_seq,
                        ...
                        if "OCs_joint" in vis_args.aggregation_strategies_to_plot_seq:
                            ...
                            "TSNE - OCs joint embedding (concatenation) - Cantagallo Sequence"
                            plot_manifold_latents(data_training_args, vis_args, decomp_args.decomp_to_perform, BETAS, data_subset, config,
                                X=mu_OCs_joint_seq_reduced, OCs=None, y_vec=cantagallo_seq,
                                target="cantagallo_seq",
                                data_set=data_training_args.dataset_name + '_' + data_subset + '_' +  str(vis_args.latent_train_set_seq_to_vis) + '_seqs',
                                manifold_dict=manifold_dict, embedding='OCs_joint_emb', group='cantagallo_seq')
