...
def main():
    ...
    for ckp_dir in checkpoints:
        ...
        "The collator only needs the frame grid, which the parameter-free geometry carries, so"
        "worker processes never receive the model's weights"
        data_collator = DataCollatorForDecVAELatentPostAnalysis_NoFeatureExtraction(
            model=representation_function.geometry,
