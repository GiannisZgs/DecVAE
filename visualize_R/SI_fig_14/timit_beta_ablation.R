#' SI Figure 14a: post-training evaluation on TIMIT. Ablation of the beta value 
#' w.r.t. disentanglement metrics.
...
models <- c("vae1D_FC","filter","ewt")
display_model_names <- c("β-VAE", "β-DecVAE + FD","β-DecVAE + EWT")
branches <- c("Z_branch")

display_metric_names <- c( "Robustness", "1 - Mutual Info.",  "1 - Gaussian Correlation", "Disentanglement", "Completeness", "Modularity", "Informativeness", "Explicitness") 
...
# For X_VAE source files, override the model to be vae1D_FC (β-VAE)
dat$Model[dat$Source == "X_VAE"] <- "vae1D_FC"
...
# Create combination labels for grouping with proper beta formatting
dat$Model_Beta <- paste0(dat$Model_Display, ", β = ", dat$Beta)

# Create proper ordering based on display_model_names and beta values
model_beta_order <- c()
for (model_display in display_model_names) {
  # For each model, add all beta combinations in ascending order
  for (beta in sort(selected_betas)) {
    combo_name <- paste0(model_display, ", β = ", beta)
    if (combo_name %in% unique(dat$Model_Beta)) {
      model_beta_order <- c(model_beta_order, combo_name)
    }
  }
}

# Create grouped bar chart for each branch
...
for (branch in unique_branches) {
  ...
  # Ensure proper ordering of groups
  subset_data$Model_Beta <- factor(subset_data$Model_Beta, 
                                   levels = model_beta_order)
  subset_data$Metric_Display <- factor(subset_data$Metric_Display, 
                                       levels = display_metric_names)
  
  p <- ggplot(subset_data, aes(x = Model_Beta, y = Value, fill = Metric_Display)) +
  ...
  save_path_plot <- file.path(save_dir, paste0("beta_ablation_grouped_", branch_clean, ".png"))
  ggsave(filename = save_path_plot, plot = p, width = 16, height = 8, dpi = 600, bg = "white")
  cat("Saved grouped bar chart for", branch, "to:", save_path_plot, "\n")
}
