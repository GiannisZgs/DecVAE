#' SI Figure 12: post-training evaluation on SimVowels. Ablation of the beta value 
#' w.r.t. disentanglement and supervised metrics.
...
# Load data from
parent_load_dir <- file.path('..', 'data', 'results_wandb_exports_for_figures', 'vowels_posttraining')
current_script_experiment <- "beta_ablation"
selected_betas <- c(0,0.1,1,5,10,20,40)
use_row <- -2
source_files <- c("all","X_VAE")
models <- c("filter","ewt","emd","vmd","vae_1d_fc")
display_model_names <- c("β-DecVAE + FD","β-DecVAE + EWT","β-DecVAE + EMD","β-DecVAE + VMD","β-VAE")
branches <- c("Z_branch")

display_metric_names <- c("Accuracy (speaker)", "Accuracy (vowel)", "Robustness", "1 - Gaussian Correlation Norm.", "1 - Mutual Information", "Disentanglement", "Completeness", "Modularity", "Informativeness", "Explicitness") 
true_metric_names <- c("accuracy_speaker","accuracy_vowel","IRS","gaussian_tc_norm","mi","disentanglement","completeness","modularity","informativeness","explicitness")
metric_y_limits <- list(
  "accuracy_speaker" = c(0.35,1),
  "accuracy_vowel" = c(0.7,1),
  "IRS" = c(0.4,1),
  "mi" = c(0.5,1),
  "gaussian_tc_norm" = c(0,1),
  "disentanglement" = c(0,0.65),
  "completeness" = c(0.1,0.7),
  "modularity" = c(0.35,1),
  "informativeness" = c(0.3,0.9),
  "explicitness" = c(0.55,1)
)

# Save data at
parent_save_dir <- file.path('..','supplementary_figures','SI_fig_12_vowels_post-training_beta_ablation')
...
# Collect all data across source files and branches
all_rows <- list()
...
for (source_file in source_files) {
  for (branch in branches) {
    ...
    for (f in files) {
      ...
      # Check if this file contains the source file name
      if (!grepl(source_file, base, ignore.case = TRUE)) {
        next
      }
      ...
      cat("Processing:", basename(f), "for source:", source_file, "branch:", branch, "metric:", metric, "\n")
      ...
      cols <- colnames(df)
      for (col in cols) {
        # Extract beta and model from column name
        beta_val <- extract_beta_from_column(col)
        model_val <- extract_model_from_column(col, models)
        
        if (is.null(beta_val) || is.null(model_val)) {
          next
        }
        
        # Check if this beta is in our selected list
        if (!(beta_val %in% selected_betas)) {
          next
        }
...
# Map model and metric names - treat X_VAE source as β-VAE model
model_mapping <- setNames(display_model_names, models)
metric_mapping <- setNames(display_metric_names, true_metric_names)

# For X_VAE source files, override the model to be vae_1d_fc (β-VAE)
dat$Model[dat$Source == "X_VAE"] <- "vae_1d_fc"
...
# Create plots for each branch-metric combination
unique_branches <- unique(dat$Branch)
unique_metrics <- unique(dat$Metric_Display)

for (branch in unique_branches) {
  for (metric_name in unique_metrics) {
    subset_data <- dat %>% 
      filter(Branch == branch, Metric_Display == metric_name)
    ...
    # Create colors for different models
    unique_models <- unique(subset_data$Model_Display)
    n_models <- length(unique_models)
    model_colors <- viridis(n_models, option = "turbo", end = yellow_block_threshold)
    names(model_colors) <- unique_models
    
    p <- ggplot(subset_data, aes(x = factor(Beta, levels = selected_betas), y = Value, color = Model_Display, group = Model_Display)) +
      geom_line(size = line_size, alpha = 0.8) +
      geom_point(size = point_size, alpha = 0.9) +
      scale_color_manual(values = model_colors) +
      scale_x_discrete(
        labels = selected_betas
      ) +
    ...
    # Save individual plot (no source in filename since combined)
    branch_clean <- str_remove(branch, "_branch")
    metric_clean <- gsub("[^A-Za-z0-9]", "_", metric_name)
    save_path_plot <- file.path(save_dir, paste0("beta_ablation_", branch_clean, "_", metric_clean, ".png"))
    ggsave(filename = save_path_plot, plot = p, width = 14, height = 8, dpi = 600, bg = "white")
    cat("Saved plot for", branch, metric_name, "to:", save_path_plot, "\n")
  }
}

cat("Completed beta ablation analysis\n")