...
plot_margin <- margin(2, 2, 2, 2)  # Reduced from (8, 8, 8, 8)
plot_margin_small <- margin(1, 1, 1, 1)  # Reduced from (5, 5, 5, 5)

parent_save_dir <-  file.path('..','figures','latent_traversals')
dataset <- 'iemocap'
model <- 'vae1d_fc' #'vae1d_fc', 'ewt', 'filter'
...
betas <- 'b1' # bz1_bs1 for DecVAEs or b1 for VAEs
      ...
      scene = list(
        ...
        zaxis = list(
          title = list(
            ...
            standoff = 50  # Increased from 15 to move title further away
          ),
          range = z_limits,
          tickfont = list(
            family = plot_font_family, 
