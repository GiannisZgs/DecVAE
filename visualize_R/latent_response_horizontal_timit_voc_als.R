...
dataset <- 'timit'
model <- 'vae1d_fc' #'vae1D_FC_mel' vae1d_fc
transfer_from <- 'sim_vowels' # timit, sim_vowels / for VOC_ALS and iemocap
experiment <- 'fixed_speaker' # fixed_phoneme, fixed_speaker, fixed_emotion_phoneme_speaker
emotion <- 'ang' # only needed for iemocap fixed_emotion_phoneme_speaker
# 'ang', 'hap', 'neu', 'sad'
feature <- 'mel'
NoC <- 4
dec_vae_type <- 'dual' # single_z, single_s, dual
betas <- 'b0' # bz1_bs1 for DecVAEs or b1 for VAEs - 'bz1_bs1'
ckp <- 'training_ckp_epoch_99'
latent <- 'X' #set to X, then function will find all available latents in the same directory

#available experiments:
# vowels - fixed_vowels_5, fixed_vowels_8, fixed_speakers_free_vowels_5, fixed_speakers_free_vowels_8
# timit - fixed_phoneme, fixed_speaker
# VOC_ALS - fixed_phoneme, fixed_kings_stage
# iemocap - fixed_emotion_phoneme_speaker, fixed_speaker_emotion, fixed_phoneme_emotion
...
#data_dir <- file.path(ckp_dir,paste0(latent,'_varying_',varying,'_fixed_',fixed,'.json'))
#save_path <- file.path(save_dir, paste0('stacked_varying_',varying,'_fixed', fixed,'_horizontal_norm.png'))
