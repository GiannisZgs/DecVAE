#' Figure 6: This script generates figures 6c in the main paper,
#' and supplementary figure 21 in the supplementary material.
...
axis_title_size <- 25        # Used for all axis titles
comparison_axis_title_size <- 25
axis_text_size <- 20         # Base size for axis text
comparison_axis_text_size <- 30
...
# Plot margins - reduce these to minimize white space
plot_margin <- margin(2, 2, 2, 2)  
plot_margin_small <- margin(1, 1, 1, 1)  

parent_save_dir <-  file.path('..','figures','fig_6','fig_6c_SI_fig_21_latent_traversals_iemocap')
dataset <- 'iemocap'
model <- 'ewt' #'vae1d_fc', 'ewt', 'filter'
      ...
      scene = list(
        xaxis = list(
          title = list(
            ...
            standoff = 50  # Increased standoff to push title further away
          ),
          ...
          linecolor = 'black',
          showgrid = TRUE,
          standoff = 50    # Push labels away from the axis
        ),
        yaxis = list(
          title = list(
            text = "Speaker",
  ...
  for (emotion_idx in seq_along(available_emotions)) {
    ...
    for (latent_type_idx in seq_along(available_latent_types)) {
      ...
      if (!emotion_name %in% names(all_snapshots[[latent_type]])) {
        ...
        empty_plot <- ggplot() + 
          theme_void() + 
          labs(title = if(is_first_row) paste0(latent_type, " - No Data") else "")
      ...
      if (is.null(first_dim_plot) || !inherits(first_dim_plot, "ggplot")) {
        empty_plot <- ggplot() + 
          theme_void() + 
          labs(title = if(is_first_row) paste0(latent_type, " - Invalid Data") else "")
        
        emotion_plots[[latent_type]] <- empty_plot
        next
      }
      
      # Use the plot as is but add a title only for the first row
      modified_plot <- first_dim_plot +
        labs(title = if(is_first_row) latent_type else "") +
        theme(
          ...
          axis.text.y = element_text(
            ...
            size = comparison_axis_text_size,
            family = plot_font_family
          ),
...
create_emotion_latent_heatmap_comparison_plot <- function(all_heatmaps, save_dir) {
  ...
  tryCatch({
    ggsave(
      save_path, 
      combined_plot, 
      width = 7 * length(available_latent_types),  # Increased from 7
      height = 5 * length(available_emotions),      # Increased from 5
      dpi = 600, 
      bg = plot_background_color
    )
            ...
            if (file.exists(snapshot_file) && requireNamespace("png", quietly = TRUE) && requireNamespace("grid", quietly = TRUE)) {
              ...
              snapshot_plot <- tryCatch({
                p <- ggplot() + 
                  ...
                  labs(title = "") + #paste0("z_", dimension_value, if(is_unused) " (unused)" else "")
                  theme_void() +
                  theme(
                    plot.title = element_text(size = plot_title_size, hjust = plot_title_hjust, family = plot_font_family)
                  )
...
# Function to create a combined plot from a nested list structure - with larger plots
create_combined_plot_by_emotion <- function(plot_list, title_prefix, save_suffix, varying_factor, emotion_name) {
  ...
  if (!is.null(combined_plot)) {
    ...
    tryCatch({
      # Increased width significantly and adjusted height
      ggsave(save_path, combined_plot, width = 32, height = 8 * length(emotion_rows), dpi = 600, bg = plot_background_color)
...
extract_2D_legend <- function(data, latent_type, emotion_name, save_dir) {
  ...
  ggsave(legend_file, legend_plot, width = 10, height = 2, dpi = 600, bg = plot_background_color)
...
extract_2D_speaker_legend <- function(data, latent_type, emotion_name, save_dir) {
  ...
  # Calculate number of items per column for balanced layout
  n_items <- length(unique_speakers)
  items_per_col <- ceiling(n_items / 2)
  ...
  dummy_data <- data.frame(
    ...
    # Add column indicator for legend placement
    column = rep(c(1, 2), length.out = length(unique_speakers))
  )
  ...
  temp_plot <- ggplot(dummy_data, aes(x = x, y = y, color = speaker)) +
    ...
    guides(
      color = guide_legend(
        ...
        byrow = TRUE      # Fill by row rather than by column
      )
    ) +
    theme(
      ...
      legend.background = element_rect(fill = plot_background_color),
      # Set horizontal alignment based on column position
      legend.text.align = 0,      # This will left-align text in the left column
      # The right column alignment will be handled by the custom legend key
      legend.spacing.x = unit(2, "cm"),  # Add more space between columns
  ...
  ggsave(legend_file, legend_plot, width = 12, height = 10, dpi = 600, bg = plot_background_color)
...
extract_2D_phoneme_legend <- function(data, latent_type, emotion_name, save_dir) {
  ...
  # Create color values for the phonemes based on selected palette option
  if (custom_palette_option == 1) {
    color_values <- viridis::turbo(n = length(unique_phonemes), begin = 0.0, end = 0.4, direction = 1)
  } else if (custom_palette_option == 2) {
    color_values <- viridis::turbo(n = length(unique_phonemes), begin = 0.6, end = 1.0, direction = 1)
  } else {
    color_values <- colorRampPalette(c(
      viridis::turbo(n = 100, begin = 0.0, end = 0.4),
      viridis::turbo(n = 100, begin = 0.6, end = 1.0)
    ))(length(unique_phonemes))
  }
  
  # Split into two columns
  n_phonemes <- length(unique_phonemes)
  half_point <- ceiling(n_phonemes / 2)
  
  # Create legend file path
  legend_file <- file.path(save_dir, paste0(latent_type, "_", emotion_name, "_2D_phoneme_legend.png"))
  
  # Set up the plotting device with high resolution
  png(legend_file, width = 12, height = 10, units = "in", res = 300, bg = "white")
  ...
  # Add title
  text(x = 0.5, y = 0.95, labels = "Phoneme", cex = 2.5, font = 1)
  
  # Calculate vertical spacing based on number of items
  y_spacing <- 0.8 / max(half_point, length(unique_phonemes) - half_point)
  
  # Draw left column (labels on LEFT of dots)
  for (i in 1:half_point) {
    y_pos <- 0.85 - (i - 1) * y_spacing
    
    # Draw the label on the LEFT of the dot
    text(x = 0.4, y = y_pos, labels = phoneme_labels[i], 
         adj = 1, cex = 2.5, col = "black", font = 1)
    
    # Draw the colored point on the RIGHT of the label
    points(x = 0.45, y = y_pos, pch = 19, cex = 3, col = color_values[i])
  }
  
  # Draw right column (labels on RIGHT of dots)
  for (i in (half_point+1):n_phonemes) {
    j <- i - half_point
    y_pos <- 0.85 - (j - 1) * y_spacing
    
    # Draw the colored point on the LEFT
    points(x = 0.55, y = y_pos, pch = 19, cex = 3, col = color_values[i])
    
    # Draw the label on the RIGHT of the dot
    text(x = 0.6, y = y_pos, labels = phoneme_labels[i], 
         adj = 0, cex = 2.5, col = "black", font = 1)
  }
...
extract_3D_heatmap_legend <- function(data, latent_type, emotion_name, save_dir) {
  ...
  ggsave(legend_file, final_legend_plot, width = 3, height = 6, dpi = 600, bg = plot_background_color)
