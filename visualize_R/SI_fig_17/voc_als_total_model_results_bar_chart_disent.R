#' SI Figure 17: disentanglement evaluation results on the VOC-ALS dataset.
...
use_row <- -1
models <- c("raw_mels","ICA","PCA","VAE_vowels","VAE_timit","filter_vowels","ewt_vowels","filter_timit","ewt_timit")
display_model_names <- c("Raw Mel Fbank","ICA","PCA","Vowels β-VAE", "TIMIT β-VAE", "Vowels β-DecVAE + FD","Vowels β-DecVAE + EWT", "TIMIT β-DecVAE + FD","TIMIT β-DecVAE + EWT")
...
# Save data at
parent_save_dir <- file.path('..','supplementary_figures','SI_fig_17_disentanglement_barcharts','VOC_ALS')
  ...
  for (col in cols) {
    ...
    if (model_val %in% c("filter_vowels", "ewt_vowels", "filter_timit", "ewt_timit", "VAE_vowels", "VAE_timit")) {
      ...
      if (model_val %in% c("filter_vowels", "ewt_vowels", "filter_timit", "ewt_timit")) {
        ...
        # Filter components based on model type
        if (grepl("vowels", model_val) && components_val != 3) {
          next  # Skip if vowels model but not NoC 3
        }
        if (grepl("timit", model_val) && components_val != 4) {
          next  # Skip if timit model but not NoC 4
        }
...
save_path_plot <- file.path(save_dir, "total_model_results_grouped_disent.png")
