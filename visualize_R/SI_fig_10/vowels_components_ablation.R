#' SI Figure 10: post-training evaluation on TIMIT. Ablation of the number of components 
...
use_row <- -2
branch <- "S_branch"
display_combination_names <- c("FD NoC 2", "FD NoC 3", "FD NoC 4", "EWT NoC 2", "EWT NoC 3", "EWT NoC 4")
true_combination_names <- c("filter_NoC2", "filter_NoC3", "filter_NoC4", "ewt_NoC2", "ewt_NoC3", "ewt_NoC4")
...
# Save data at
parent_save_dir <- file.path('..','supplementary_figures','SI_fig_10_vowels_post-training_components_ablation')
...
for (f in files) {
  ...
  for (combo in true_combination_names) {
    # Create pattern for this combination (decomp_NoC pattern)
    combo_parts <- strsplit(combo, "_")[[1]]
    decomp_part <- combo_parts[1]
    noc_part <- combo_parts[2]
    
    # Look for columns that match both beta and combination pattern
    combo_pattern <- paste0("(", decomp_part, "|", toupper(decomp_part), ").*", noc_part)
    combo_cols <- beta_match_cols[grepl(combo_pattern, beta_match_cols, ignore.case = TRUE)]
    
    if (length(combo_cols) > 0) {
      # Take first matching column and extract value
      selected_col <- combo_cols[1]
