...
load_all_latent_types <- function(file_path) {
  ...
  for (latent in base_latent_types) {
    # Check and load total MI matrix using pairwise structure for VOC_ALS and IEMOCAP
    if (!is.null(all_data[[latent]])) {
      
      # For VOC_ALS and IEMOCAP, use pairwise MI matrices
      if (grepl("voc_als", dataset)) {
        # Use phonemes_speakers pairwise matrix as the primary total MI matrix
        total_mi <- all_data[[latent]]$pairwise_mi$phonemes_speakers
      } else if (grepl("iemocap", dataset)) {
        # Use phonemes_speakers for frame-level or speakers_emotions for sequence-level
        if (grepl("sequence", analysis_level)) {
          total_mi <- all_data[[latent]]$pairwise_mi$speakers_emotions
        } else {
          total_mi <- all_data[[latent]]$pairwise_mi$phonemes_speakers
        }
      } else {
        # Fallback to original structure for other datasets
    ...
    # Check and load instance MI matrices using pairwise structure
    instance_key <- paste0(latent, "_instance")
    
    if (!is.null(all_data[[latent]]$pairwise_mi_instance)) {
      result[[instance_key]] <- list()
      
      # Phoneme instance MI
      if (!is.null(all_data[[latent]]$pairwise_mi_instance$phonemes_speakers$phonemes$matrix)) {
        phoneme_mi <- all_data[[latent]]$pairwise_mi_instance$phonemes_speakers$phonemes$matrix
      ...
      # Speaker instance MI
      if (!is.null(all_data[[latent]]$pairwise_mi_instance$phonemes_speakers$speakers$matrix)) {
        speaker_mi <- all_data[[latent]]$pairwise_mi_instance$phonemes_speakers$speakers$matrix
      ...
      # VOC_ALS: King stage instance MI
      if (grepl("voc_als", dataset)) {
        if (!is.null(all_data[[latent]]$pairwise_mi_instance$phonemes_king_stage$king_stage$matrix)) {
          king_stage_mi <- all_data[[latent]]$pairwise_mi_instance$phonemes_king_stage$king_stage$matrix
          if (!is.null(king_stage_mi)) {
      ...
      # IEMOCAP: Emotion instance MI
      if (grepl("iemocap", dataset)) {
        if (!is.null(all_data[[latent]]$pairwise_mi_instance$speakers_emotions$emotions$matrix)) {
          emotion_mi <- all_data[[latent]]$pairwise_mi_instance$speakers_emotions$emotions$matrix
...
create_stacked_mi_heatmaps <- function(data, mi_matrix_type = "total", global_max = NULL, instance_type = "both") {
  ...
  for (i in 1:n_latents) {
    ...
    if (mi_matrix_type == "total") {
      ...
      # Factor labels - now using standard 2-factor approach for all datasets
      if (grepl("voc_als", dataset)) {
