#' SI Figure 4: this script generates figure SI_fig_4c. Demonstrates the recovered 
#' simulated signals of the SimVowels dataset - the recognition model of the frame variable.
...
parent_data_dir <- file.path('..','data','frequency_analysis')
...
json_gz_file_path <- file.path('..','data','sim_vowels_figures.json.gz') 
json_file_path <- gsub(".gz$", "", json_gz_file_path)

# Set and create directory and filepaths to save
save_dir <-  file.path('..','supplementary_figures','SI_fig_4','recognition_model','frame')
...
cycles_to_keep <- 1 # Number of cycles to keep for visualizations
frame_to_plot <- 3 # second frame corresponds to other plots 
sample <- "_0.705" #this does not affect the output, is just a reminder of the subject that these data come from
start_ind <- 320
vowel <- "u"
savename <- file.path(save_dir, paste0("vowel_", vowel,"_vt",sample,"_recognition_model_", vowel_dur_str, "ms_frame_single_speaker.png"))
...
end_ind <- start_ind + samples_to_keep - 1
...
create_decomposition_tibbles <- function(data, vowel, frame_idx, start_ind = NULL, end_ind = NULL) {
    # Components to extract
    components <- c("Reconstructed", "OC1", "OC2", "OC3")
    spectral_components <- paste0(components, "_PSD")
    spectral_components_prev <- paste0("spectral_density_", components)
    spectral_components_prev[1] <- "spectral_density_X"
    ...
    if (is_input) {
        # For input data, use start and end indices
        time_domain_values[["Reconstructed"]] <- as.numeric(data[[vowel]][["frame"]][1,])
        for (i in 2:length(components)) {
            time_domain_values[[components[i]]] <- as.numeric(data[[vowel]][["frame"]][i,])
        }
    } else {
        # For decomposition data, use frame_idx
        for (i in 2:length(components)) {
            time_domain_values[[components[i]]] <- as.numeric(data[[vowel]][["frame"]][i, frame_idx, ])
        }
    ...
    x_spectral <- if (is_input) {
        as.numeric(data[[vowel]][["Reconstructed_PSD"]])
    } else {
        oc_spectral_values <- lapply(spectral_components_prev[-1], function(comp) {
            as.numeric(data[[vowel]][[comp]][frame_idx, ])
        })
        Reduce("+", oc_spectral_values)
    } #as.numeric(data[[vowel]][["spectral_density_X"]][frame_idx, ])
    if (is_input) {
      max_amplitude <- max(abs(x_spectral))
    } else {
      max_amplitude <- max(abs(as.numeric(data[[vowel]][["spectral_density_X"]][frame_idx, ])))
    }
    ...
    for (i in 1:length(spectral_components_prev)) {
        values <- if (is_input) {
            ...
            as.numeric(data[[vowel]][[spectral_components_prev[i]]][frame_idx, 1:floor(length(x_spectral)/2+1)])
        }
...
input <- list()
input[[vowel]] <- list(
    frame = data_raw[["filter"]][[vowel]][["input_frame"]],
    frequencies = data_raw[["filter"]][[vowel]][["frequencies"]],
    Reconstructed_PSD = data_raw[["filter"]][[vowel]][["spectral_density_input_frame_X"]],
    OC1_PSD = data_raw[["filter"]][[vowel]][["spectral_density_input_frame_OC1"]],
    OC2_PSD = data_raw[["filter"]][[vowel]][["spectral_density_input_frame_OC2"]],
    OC3_PSD = data_raw[["filter"]][[vowel]][["spectral_density_input_frame_OC3"]]
)

# Merge input with data_raw
data_raw <- c(list(input=input),data_raw)

original_data <- create_decomposition_tibbles(data_raw[["input"]], vowel, frame_to_plot)
filter_data <- create_decomposition_tibbles(data_raw[["filter"]], vowel, frame_to_plot)
vmd_data <- create_decomposition_tibbles(data_raw[["vmd"]], vowel, frame_to_plot)
vmd_corrected_data <- create_decomposition_tibbles(data_raw[["vmd_corrected"]], vowel, frame_to_plot)
emd_data <- create_decomposition_tibbles(data_raw[["emd"]], vowel, frame_to_plot)
ewt_data <- create_decomposition_tibbles(data_raw[["ewt"]], vowel, frame_to_plot)
...
create_signal_plot <- function(decomposition_method, domain = "time_domain", ymin, ymax, 
  ...
  ggplot(data) +
    geom_hline(yintercept = 0, linetype="solid", size=line_size_thin) +
    geom_line(aes(x=time, y=value, color=signal_type)) +  
