...
class FrozenSSLArguments:
    ...
    ssl_seq_pooling: str = field(
        default=None,
        metadata={"help": "How the frame embeddings of an utterance are pooled into one sequence-level embedding, "
                          "for the '*_seq' classification targets. None skips them. 'mean' averages over every "
                          "frame of the padded sequence, which is what DecVAE's SequenceAggregator does, so the "
                          "two stay comparable."},
    )
