...
def prepare_data_for_quality_assessment(batch, feature_extractor, data_training_args, decomp_args, config, max_length):
    """ 
    ...
    and returns all necessary data in batched format per sample-sequence. Returns some extra information for the decomposition quality assessment
    experiment - reconstruction NRMSEs and correlograms.
    ...
    if data_training_args.dataset_name == "timit":
        ...
        def truncate_from_middle(audio_array, sampling_rate, seconds_to_skip, max_length=None):
            ...
            # Calculate how many samples to skip (1 second worth)
            samples_to_skip = int(seconds_to_skip*sampling_rate)  # 1 second * sampling_rate
        ...
        "Also encode healty control labels from None or - to a number e.g. -1 or 100"
    ...
    if data_training_args.dataset_name in ["timit", "VOC_ALS", "iemocap"]:
        ...
        decomposition_outcome, _, _, _, reconstruction_NRMSEs, reconstruction_NRMSEs_seq, avg_correlogram, correlograms_torch, avg_correlogram_seq, _, _, _, _ = decomp_module(
    ...
    #batch["input_dec_values"] = decomposition_outcome[1:,:,:,:].squeeze(1)
    batch["input_length"] = max_length #len(inputs.input_values[0])
    batch["attention_mask"] = attention_mask
    batch["reconstruction_NRMSEs"] = reconstruction_NRMSEs #+ [np.nan]*(mask_indices_seq_length-len(reconstruction_NRMSEs))
    batch["reconstruction_NRMSE_seq"] = reconstruction_NRMSEs_seq
    batch["correlograms"] = correlograms_torch
    batch["correlogram_seq"] = avg_correlogram_seq
