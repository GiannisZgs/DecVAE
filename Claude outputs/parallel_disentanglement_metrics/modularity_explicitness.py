...
def compute_modularity_explicitness(mus_train, ys_train, mus_test, ys_test, use_cv = False, n_splits = 5,n_random_states = 5, n_jobs = 1):
  ...
  if use_cv:
    "Calculate modularity and explicitness in KFold CV with random states"
    ...
    # With the cells spread over n_jobs workers, each cell's mutual information runs on one thread
    mi_jobs = 1 if n_jobs > 1 else n_jobs

    # Parallel returns results in submission order, whatever order the cells finish in
    cell_results = Parallel(n_jobs=n_jobs)(
        delayed(_modularity_explicitness_cell)(mus_combined, ys_combined, train_idx, test_idx,
                                               rs, fold, n_random_states, n_splits, mi_jobs)
        for rs, fold, train_idx, test_idx in cells
    )
    modularity_scores = [r[0] for r in cell_results]
    explicitness_train_scores = [r[1] for r in cell_results]
    explicitness_test_scores = [r[2] for r in cell_results]

    "Calculate statistics"
    modularity_mean = np.mean(modularity_scores)
    modularity_std = np.std(modularity_scores)
    ...
    modularity_ci = z_score * (modularity_std / np.sqrt(n_samples))
    ...
    scores.update({
        "modularity_score": modularity_mean,
        "modularity_std": modularity_std,
        "modularity_ci": modularity_ci,
        "modularity_values": modularity_scores,
    ...
    print("\nModularity & Explicitness Cross-validation Results:")
    print("------------------------------------------------")
    print(f"Modularity: {modularity_mean:.4f} ± {modularity_ci:.4f} (95% CI)")
    ...
    mutual_information = disentanglement_eval.discrete_mutual_info(discretized_mus, ys_combined, n_jobs=n_jobs)
...
def _modularity_explicitness_cell(mus_combined, ys_combined, train_idx, test_idx, rs, fold,
                                  n_random_states, n_splits, n_jobs):
  "Modularity and explicitness of one (random state, fold) cell of the cross-validation grid"
  print(f"\nRandom state {rs+1}/{n_random_states} - Processing fold {fold+1}/{n_splits}")
  ...
  # Calculate modularity for this fold, on its training portion
  discretized_mus = disentanglement_eval.histogram_discretize(fold_mus_train)
  mutual_information = disentanglement_eval.discrete_mutual_info(discretized_mus, fold_ys_train, n_jobs=n_jobs)
  assert mutual_information.shape[0] == fold_mus_train.shape[0]
  assert mutual_information.shape[1] == fold_ys_train.shape[0]
  fold_modularity = modularity(mutual_information)
  ...
  print(f"    Modularity: {fold_modularity:.4f}")
  print(f"    Explicitness (train): {fold_explicitness_train:.4f}")
  print(f"    Explicitness (test): {fold_explicitness_test:.4f}")
  return fold_modularity, fold_explicitness_train, fold_explicitness_test
