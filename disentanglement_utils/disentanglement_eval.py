...
def _adjusted_mutual_info(codes_u, k_u, h_u, codes_v, k_v, h_v, gln, lgam):
  ...
  if (k_u == 1 and k_v == 1) or (k_u == 0 and k_v == 0):
    return 1.0
  if k_u == 1 or k_v == 1:
    return 0.0
...
def discrete_mutual_info(mus, ys, n_jobs=1):
  ...
  # The input checks sklearn ran on every pair: finite labels and a matching number of points
  if num_codes and num_factors:
    for x in (ys, mus):
      if np.issubdtype(x.dtype, np.floating) and not np.all(np.isfinite(x)):
        raise ValueError("Input contains NaN or infinity.")
      if x.dtype == object and np.any(x != x):
        raise ValueError("Input contains NaN.")
    if mus.shape[1] != ys.shape[1]:
      raise ValueError(f"Found input variables with inconsistent numbers of samples: [{ys.shape[1]}, {mus.shape[1]}]")
