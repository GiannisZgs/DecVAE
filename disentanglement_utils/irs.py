...
def compute_irs(mus,ys,diff_quantile=0.99):
  ...
  if not disentanglement_eval.is_discrete(ys):
    ys = disentanglement_eval.histogram_discretize(ys)
  active_mus = _drop_constant_dims(mus)

  if not active_mus.any():
    irs_score = 0.0
  else:
    irs_dict = scalable_disentanglement_score(ys.T, active_mus.T,
                                               diff_quantile)

  score_dict = {}
  score_dict["IRS"] = irs_dict["avg_score"]
  score_dict["disentanglement_scores"] = irs_dict["disentanglement_scores"]
  score_dict["parents"] = irs_dict["parents"]
  score_dict["IRS_matrix"] = irs_dict["IRS_matrix"]
  score_dict["max_deviations"] = irs_dict["max_deviations"]
